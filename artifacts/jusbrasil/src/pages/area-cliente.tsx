...
import {
  ...
  Loader2, AlertCircle, Scale, Users,
} from "lucide-react";
...
export default function AreaClientePage() {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    else { setUploadedFiles(prev => [...prev, file.name]); }
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm">
            ...
            {loadingCase ? (
              ...
              <div className="space-y-3">
                <div><p className="text-xs text-muted-foreground font-medium mb-0.5">Nome</p><p className="text-sm font-semibold">{caseData.full_name}</p></div>
                <div><p className="text-xs text-muted-foreground font-medium mb-0.5">Descrição</p><p className="text-sm leading-relaxed">{caseData.case_description}</p></div>
                {caseData.claim_value != null && caseData.claim_value > 0 && (
                  <div><p className="text-xs text-muted-foreground font-medium mb-0.5">Valor Buscado</p>
                    <p className="text-sm font-semibold text-emerald-700">R$ {caseData.claim_value.toLocaleString("pt-BR", { minimumFractionDigits: 2 })}</p>
                  </div>
                )}
                {(caseData.city || caseData.state) && (
                  <div><p className="text-xs text-muted-foreground font-medium mb-0.5">Localização</p>
                    <p className="text-sm">{[caseData.city, caseData.state].filter(Boolean).join(", ")}</p>
                  </div>
                )}
                <div><p className="text-xs text-muted-foreground font-medium mb-0.5">Enviado em</p>
