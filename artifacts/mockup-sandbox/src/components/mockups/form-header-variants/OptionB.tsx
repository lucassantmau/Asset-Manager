import { Clock, ShieldCheck, AlertCircle, CheckCircle, Circle, ArrowRight } from "lucide-react";
  ...
  return (
    <div className="min-h-screen bg-slate-100 flex items-start justify-center p-8">
      <div className="w-full max-w-lg bg-white rounded-xl border-4 border-[#e5e7eb] shadow-xl overflow-hidden">
        ...
        {/* OPTION B — Cobalt bar */}
        <div className="bg-[hsl(214,82%,46%)] px-6 py-4 flex items-center justify-between">
          <h3 className="text-xl font-bold text-white">Conte o que aconteceu</h3>
          <span className="inline-flex items-center gap-1.5 bg-white/15 text-white text-sm font-semibold px-3 py-1 rounded-full">
        ...
        <div className="p-6 space-y-5">
          <div>
            ...
            <textarea
              className="w-full bg-white border-[3px] border-slate-300 rounded-xl p-4 text-[#111] placeholder:text-slate-400 focus:outline-none focus:border-[hsl(214,82%,46%)] min-h-[130px] resize-y text-base"
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {["Conversas (WhatsApp)", "Áudios / Gravações", "Fotos / Vídeos", "E-mails"].map(ev => (
                <button key={ev} type="button" className="flex items-center gap-2 px-3 py-2 rounded-full text-sm font-medium border-2 border-slate-200 bg-white text-slate-500 hover:border-blue-400">
